...
app.post('/updateShipping', async (req, res) => {
    ...
    try {
        ...
        await db.execute(query, [JSON.stringify(shippingInfo), phoneNumber, userIdToUse]);
...
app.post('/updatePayment', async (req, res) => {
    ...
    try {
        ...
        await db.execute(query, [JSON.stringify(billingInfo), JSON.stringify(paymentInfo), userIdToUse]);
